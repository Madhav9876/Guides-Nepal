...
check("Sign up new user returns 200 + tokens", r.status_code == 200 and "access_token" in r.json())
check("Sign up returns a user object", r.status_code == 200 and r.json().get("user", {}).get("email") == "test1@example.com")
...
check("Signed-up user logs in successfully (200)", r.status_code == 200 and "access_token" in r.json())
