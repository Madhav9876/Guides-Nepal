...
const KathmanduExperiencePage: React.FC = () => {
  const { slug } = useParams<{ slug: string }>();
  const navigate = useNavigate();
  const [data, setData] = useState(kathmanduRichData.find(d => d.slug === slug));
  ...
  useEffect(() => {
    const found = kathmanduRichData.find(d => d.slug === slug);
    if (found) {
      setData(found);
    ...
    return (
      <div className="min-h-screen flex flex-col font-sans bg-white">
        ...
        <main className="flex-grow container mx-auto px-4 py-12 text-center">
          ...
          <button onClick={() => navigate('/city/kathmandu')} className="text-primary hover:underline mt-4">
            Back to Kathmandu
          </button>
        </main>
        <Footer />
              ...
              <div className="sticky top-24 space-y-8">
                ...
                <div>
                  ...
                  <div className="space-y-6">
                    {kathmanduRichData.filter(d => d.id !== data.id).slice(0, 4).map((item, i) => (
                      <div 
                        key={i} 
                        className="flex gap-4 group cursor-pointer"
                        onClick={() => {
                          navigate(`/city/kathmandu/experience/${item.slug}`);
...
export default KathmanduExperiencePage;
