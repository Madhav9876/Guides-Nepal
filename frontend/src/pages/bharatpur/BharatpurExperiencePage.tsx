...
import { Guide } from '../../data/kathmanduRichData'; // Reuse Guide type
...
const BharatpurExperiencePage: React.FC = () => {
  const { slug } = useParams<{ slug: string }>();
  const navigate = useNavigate();
  const [data, setData] = useState(bharatpurRichData.find(d => d.slug === slug));
  ...
  useEffect(() => {
    const found = bharatpurRichData.find(d => d.slug === slug);
    if (found) {
      setData(found);
    ...
    return (
      <div className="min-h-screen flex flex-col font-sans bg-white">
        ...
        <main className="flex-grow container mx-auto px-4 py-12 text-center">
          ...
          <button onClick={() => navigate('/city/bharatpur')} className="text-primary hover:underline mt-4">
            Back to Bharatpur
          </button>
        </main>
        <Footer />
              ...
              <div className="sticky top-24 space-y-8">
                ...
                <div>
                  ...
                  <div className="space-y-6">
                    {bharatpurRichData.filter(d => d.id !== data.id).slice(0, 4).map((item, i) => (
                      <div 
                        key={i} 
                        className="flex gap-4 group cursor-pointer"
                        onClick={() => {
                          navigate(`/city/bharatpur/experience/${item.slug}`);
...
export default BharatpurExperiencePage;
