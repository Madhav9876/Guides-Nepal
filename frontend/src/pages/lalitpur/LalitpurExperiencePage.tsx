...
const LalitpurExperiencePage: React.FC = () => {
  const { slug } = useParams<{ slug: string }>();
  const navigate = useNavigate();
  const [data, setData] = useState(lalitpurRichData.find(d => d.slug === slug));
  ...
  useEffect(() => {
    const found = lalitpurRichData.find(d => d.slug === slug);
    if (found) {
      setData(found);
    ...
    return (
      <div className="min-h-screen flex flex-col font-sans bg-white">
        ...
        <main className="flex-grow container mx-auto px-4 py-12 text-center">
          ...
          <button onClick={() => navigate('/city/lalitpur')} className="text-primary hover:underline mt-4">
            Back to Lalitpur
          </button>
        </main>
        <Footer />
            ...
            <div className="lg:col-span-1">
              <div className="sticky top-24 space-y-8">
                ...
                <div className="space-y-6">
                  {lalitpurRichData.filter(d => d.id !== data.id).slice(0, 4).map((item, i) => (
                    <div 
                      key={i} 
                      className="flex gap-4 group cursor-pointer"
                      onClick={() => {
                        navigate(`/city/lalitpur/experience/${item.slug}`);
...
export default LalitpurExperiencePage;
