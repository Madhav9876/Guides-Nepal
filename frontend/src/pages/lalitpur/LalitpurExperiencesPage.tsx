...
export const LalitpurExperiencesPage: React.FC = () => {
  const navigate = useNavigate();
  const [filterType, setFilterType] = useState('All');

  const filteredData = filterType === 'All' 
    ? lalitpurRichData 
    : lalitpurRichData.filter(item => item.type?.includes(filterType) || item.title.includes(filterType));
    ...
    <div className="min-h-screen flex flex-col font-sans bg-gray-50">
      ...
      <main className="container mx-auto px-4 py-8 max-w-7xl">
        <div className="mb-8">
           ...
           <h1 className="text-3xl font-bold text-gray-900 mb-2">Experiences in Lalitpur</h1>
           <p className="text-gray-500">Discover {lalitpurRichData.length} unique activities hosted by locals</p>
        </div>
        ...
        <div className="flex flex-wrap gap-2 mb-8 sticky top-20 z-10 bg-gray-50/95 backdrop-blur-sm py-4 -mx-4 px-4 border-b border-gray-200/50">
           {['All', 'Food', 'Art', 'Culture', 'History'].map(type => (
              <button 
                key={type}
...
export default LalitpurExperiencesPage;
